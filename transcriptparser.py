...
def run(filename): #example input: captions.en.csv
	...
	htmltext += '</div>'
	htmltext += '<script>var MarkersInit'+tracknum+' = function(markers) { var elements'+tracknum+' = document.querySelectorAll(".wavesurfer-marker"); Array.prototype.forEach.call(elements'+tracknum+', function(el, i) { var time_start = el.dataset.start; var time_end = el.dataset.end; var id = el.dataset.id;; if (id >= 1) { id = id - 1; } else { id = 0; } marker = {}; marker.time_start = time_start; marker.time_end = time_end; marker.dom = el; if (typeof(markers[id]) === "undefined") { markers[id] = []; } markers[id].push(marker); }); } document.onreadystatechange = () => { if (document.readyState === "complete") { var markers = []; MarkersInit'+tracknum+'(markers); var iframeElement'+tracknum+' = document.querySelectorAll("sound'+tracknum+'"); var widget'+tracknum+' = SC.Widget(iframeElement'+tracknum+'); var elements'+tracknum+' = document.querySelectorAll(".wavesurfer-marker"); Array.prototype.forEach.call(elements'+tracknum+', function(el, i) { el.onclick = function() { var pos = el.dataset.start * 1000 +1; widget'+tracknum+'.seekTo(pos); } }); widget'+tracknum+'.bind(SC.Widget.Events.PLAY_PROGRESS, function() { widget'+tracknum+'.getPosition(function(current_time) { current_time = current_time / 1000; var j = 0; markers[j].forEach(function(marker, i) { if (current_time >= marker.time_start && current_time <= marker.time_end) { marker.dom.classList.add("wavesurfer-marker-current'+tracknum+'"); marker.dom.scrollIntoView({ block: "nearest"}) } else { marker.dom.classList.remove("wavesurfer-marker-current'+tracknum+'"); } }); }); }); } };</script>'
