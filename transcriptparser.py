...
def run(filename): #example input: captions.en.csv
	...
	htmltext += '</div>'
	htmltext += '<script>var MarkersInit'+tracknum+'=function(markers'+tracknum+'){var elements'+tracknum+'=document.querySelectorAll(".wavesurfer-marker'+tracknum+'");Array.prototype.forEach.call(elements'+tracknum+',function(el'+tracknum+',i){var time_start=el'+tracknum+'.dataset.start;var time_end=el'+tracknum+'.dataset.end;var id=el'+tracknum+'.dataset.id;if(id>=1){id=id-1}else{id=0}
marker={};marker.time_start=time_start;marker.time_end=time_end;marker.dom=el'+tracknum+';if(typeof(markers'+tracknum+'[id])==="undefined"){markers'+tracknum+'[id]=[]}
