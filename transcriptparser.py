...
def run(filename): #example input: captions.en.csv
	...
	htmltext += '</div>'
	htmltext += '''<script>
	var MarkersInit'+tracknum+' = function(markers'+tracknum+') {
  var elements'+tracknum+' = document.querySelectorAll(".wavesurfer-marker'+tracknum+'");
  Array.prototype.forEach.call(elements'+tracknum+', function(el'+tracknum+', i) {
    var time_start = el'+tracknum+'.dataset.start;
    var time_end = el'+tracknum+'.dataset.end;
    var id = el'+tracknum+'.dataset.id;;
    if (id >= 1) {
      id = id - 1;
    } else {
      id = 0;
    ...
    marker.dom = el'+tracknum+';
    if (typeof(markers'+tracknum+'[id]) === "undefined") {
      markers'+tracknum+'[id] = [];
    }
    markers'+tracknum+'[id].push(marker);
  });
}
...
document.onreadystatechange = () => {
  if (document.readyState === "complete") {
    ...
    // Init Markers
    var markers'+tracknum+' = [];
    MarkersInit'+tracknum+'(markers'+tracknum+');

    // Create SoundCloud Player Object ( TODO: all loop)
    var iframeElement'+tracknum+' = document.querySelectorAll("sound'+tracknum+'");
    var widget'+tracknum+' = SC.Widget(iframeElement'+tracknum+');

    // Register On Click Event Handler
    var elements'+tracknum+' = document.querySelectorAll(".wavesurfer-marker'+tracknum+'");
    Array.prototype.forEach.call(elements'+tracknum+', function(el'+tracknum+', i) {
      el'+tracknum+'.onclick = function() {
        // Get Data Attribute
        var pos = el'+tracknum+'.dataset.start * 1000 +1;
        // Seek
        widget'+tracknum+'.seekTo(pos);
      }
    });

    // Bind Continuous Play
    widget'+tracknum+'.bind(SC.Widget.Events.PLAY_PROGRESS, function() {
      widget'+tracknum+'.getPosition(function(current_time) {
        current_time = current_time / 1000;
        var j = 0; // NOTE: to extend for several players
        markers'+tracknum+'[j].forEach(function(marker, i) {
          if (current_time >= marker.time_start && current_time <= marker.time_end) {
            marker.dom.classList.add("wavesurfer-marker-current'+tracknum+'");
            marker.dom.scrollIntoView({ block: "nearest"})
          } else {
            marker.dom.classList.remove("wavesurfer-marker-current'+tracknum+'");
          }
        });
      });
    });
...
}; // Document Ready State Change</script>'''
